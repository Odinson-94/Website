...
test('only published fields escape and response bypasses caches',async()=>{
 ...
 setup({retail:{currency:'GBP',packCredits:15,packPriceMinor:2000},tokenRates:[{code:'in',model:'test',tier:'standard',component:'uncached_input',context:'standard',usageCreditsPerMillion:20,secret:'secret'}],version:3,publishedAt:'2026-09-19T00:00:00Z',products:{cable:price,sap:price,lighting:price,roomplanner:price,internal:{...price,mode:'internal'},browser:{...price,mode:'included',usageCredits:null,batches:[]}},draft:{secret:true},actor:'private@test'});
 const r=await run();assert.equal(r.body.version,3);assert.equal(r.body.products.sap.usageCredits,20);assert.equal(r.body.identicalDownloadsFree,true);assert.equal(r.body.contractVersion,3);assert.equal(r.body.products.roomplanner.usageCredits,20);assert.equal(r.body.products.internal,undefined);assert.equal(r.body.products.browser.usageCredits,null);
