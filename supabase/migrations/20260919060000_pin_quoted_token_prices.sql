-- Preserve the exact provider rate timestamp used to quote the reservation.
-- Existing reservations without the server-owned timestamp retain their original behavior.
begin;
...
begin
  ...
  v_quote_at:=coalesce((v_res.metadata->>'billing_quote_at')::timestamptz,v_res.reserved_at);
  for v_component in select * from (values
    ...
    select * into v_rate from public.adelphos_provider_rate_card
      where model=v_res.model and component=v_component.component
        ...
        and active and effective_from<=v_quote_at
        and (effective_until is null or effective_until>v_quote_at)
